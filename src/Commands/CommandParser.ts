...
> = {
 ...
 data: (input, index, arg) => {
  ...
  try {
   let value: any = JSON.parse(string);
 ...
 number: (input, index, arg) => {
  ...
  let value: any = parseFloat(numString);
 ...
 "number-array": (input, index, arg) => {
  let arrayLength = (arg as CommandNumberArrayArg).length;
  ...
  while (processing) {
   ...
   index++;
   if (index >= input.length) processing = false;
   if(numString.length == 0) continue;
   let value: any = parseFloat(numString);
   if (typeof value != "number" || Number.isNaN(value)) {
    Commands.error(`${arg.fullId} must be a number`);
...
export const CommandParser = {
 ...
 parseArgs(input: string, args: CommandArguments[], index: number) {
  const data: Record<string, any> = {};

  const map: Map<string, CommandArguments> = new Map();
  ...
  for (let i = index; i < input.length; i++) {
   ...
   if (char == "-" && mode == "none") {
    ...
    if (!arg) {
     Commands.error(
      `Flag with ID "${currentFlag}" does not exists for this command.`
     );
     return null;
    }
    const valueData = GetValue[arg.type](input, index, arg);
    if (valueData.value == undefined) return null;
